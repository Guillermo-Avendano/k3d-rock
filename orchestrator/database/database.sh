...
install_database() {
    ...
    if [ "$DATABASE_PROVIDER" == "postgresql" ]; then
        ...
        cp $kube_dir/orchestrator/database/storage/local/templates/$POSTGRES_STORAGE_FILE $kube_dir/orchestrator/database/storage/local/$POSTGRES_STORAGE_FILE;

        POSTGRES_CONF_FILE=$kube_dir/orchestrator/database/$POSTGRES_VALUES_TEMPLATE;
        cp $kube_dir/orchestrator/database/templates/$POSTGRES_VALUES_TEMPLATE $POSTGRES_CONF_FILE;
        ...
        kubectl apply -f $kube_dir/orchestrator/database/storage/local/$POSTGRES_STORAGE_FILE --namespace $NAMESPACE;
